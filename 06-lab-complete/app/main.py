...
def root():
    return {
        ...
        "endpoints": {
            ...
            "ask_multi_agent_stream": "POST /ask-multi-agent/stream (requires Bearer JWT)",
...
def ready():
    ...
    qdrant_check = get_dependency_checks().get("qdrant", {"status": "degraded", "detail": "missing"})
    if redis_check.get("status") != "ok" or qdrant_check.get("status") != "ok":
        raise HTTPException(
            503,
            detail={
                "ready": False,
