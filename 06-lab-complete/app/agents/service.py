...
class AgentService:
    ...
    def warmup_regulation_store(self) -> None:
        try:
            regulation_rag.initialize()
            logger.info("Warmup regulation store completed")
        except Exception as exc:
            logger.warning("Warmup regulation store failed: %s", exc)

    def get_dependency_checks(self) -> dict:
        return {
            "qdrant": regulation_rag.health_check(),
        }
    ...
    async def stream_run(
        ...
        # yield initial event
        yield json.dumps({"event": "metadata", "data": {"status": "Khởi tạo luồng xử lý..."}}) + "\n\n"
